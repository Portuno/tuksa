...
export async function POST(req: NextRequest) {
  ...
  logWithTime("[MabotChat] ENV:", { apiBaseUrl, user: !!user, pass: !!pass });
  logWithTime("[MabotChat] Incoming message:", message, "chat_id:", chat_id);

  if (!apiBaseUrl || !user || !pass) {
    errorWithTime("[MabotChat] Configuración incompleta", { apiBaseUrl, user, pass });
  ...
  let token = null;
  try {
    logWithTime("[MabotChat] Intentando login en:", `${apiBaseUrl}/auth/login`);
    ...
    logWithTime("[MabotChat] Login status:", loginRes.status);
    const loginText = await loginRes.clone().text();
    logWithTime("[MabotChat] Login response body:", loginText);
    if (!loginRes.ok) {
      errorWithTime("[MabotChat] Login error:", loginText);
      return NextResponse.json({ error: "No se pudo autenticar con Mabot", detail: loginText }, { status: 401 });
    }
    const loginData = JSON.parse(loginText);
    token = loginData.access_token;
    logWithTime("[MabotChat] Token obtenido:", !!token);
  } catch (e) {
    errorWithTime("[MabotChat] Error de autenticación:", e);
    return NextResponse.json({ error: "Error de autenticación", detail: String(e) }, { status: 401 });
  }

  // 2. Enviar mensaje al bot
  try {
    logWithTime("[MabotChat] Enviando mensaje a:", `${apiBaseUrl}/io/input`);
    const chatId = getChatId(chat_id);
    const bodyToSend = {
      platform: "web",
      bot_username: "inmobot",
      chat_id: chatId,
      messages: [
        {
          role: "user",
    ...
    logWithTime("[MabotChat] Body enviado al bot:", JSON.stringify(bodyToSend));
    ...
    logWithTime("[MabotChat] Bot response status:", botRes.status);
    const botText = await botRes.clone().text();
    logWithTime("[MabotChat] Bot response body:", botText);
    if (!botRes.ok) {
      errorWithTime("[MabotChat] Error al contactar al bot:", botText);
    ...
    logWithTime("[MabotChat] Bot reply:", reply);
    return NextResponse.json({ reply });
  } catch (e) {
    errorWithTime("[MabotChat] Error de conexión con el bot:", e);
